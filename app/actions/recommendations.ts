...
export async function getRecommendations(
  ...
  for (const [courseGroup, entries] of Object.entries(data)) {
    ...
    // Filter courses that belong to the selected cluster
    // Note: inferCluster ensures we only look at relevant courses from the large dataset
    if (inferredId === clusterId || (clusterId === 0)) { // 0 allows selecting 'All' if implemented
